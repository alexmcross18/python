...
# Creates a variable called "log_lines" and assigns the input file (file with the logs) to it
log_lines = "logs.txt"

# Opens the logs file, in read mode and assigns the "lines" variable to read each line of the logs file
with open(log_lines, "r") as input_file:
...
# Creates a new json file "parsed_logs.json" in write mode and adds the data from the "parsed_logs" list
# Indents the json file 4 times per row of data to make it easier to read
with open("parsed_logs.json", "w") as output_file:
